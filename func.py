...
import epay
import epusdt
from config import TOKEN, ADMIN_ID, ADMIN_COMMAND_START, ADMIN_COMMAND_QUIT, PAY_TIMEOUT, EPUSDT_IS_ENABLE, \
    TELEGRAM_API_URL
from epay import make_data_dict, epay_submit
from epusdt import epusdt_submit
...
bot = telegram.Bot(token=TOKEN, base_url=TELEGRAM_API_URL)


def run_bot():
    updater = Updater(token=TOKEN, use_context=True, base_url=TELEGRAM_API_URL)
    ...
    start_handler = ConversationHandler(
        ...
        states={
            ...
            SUBMIT: [
                ...
                CallbackQueryHandler(submit_trade_usdt, pattern='^' + str('submit_order_usdt') + '$'),
    ...
    if len(active_cards) == 0 and len(locking_cards) != 0:
        ...
        keyboard = [
            [InlineKeyboardButton("提交订单", callback_data=str('提交订单')),
             InlineKeyboardButton("提交订单(USDT)", callback_data=str('submit_order_usdt')),
            ...
            if pay_url != 'API请求失败':
                ...
                cursor.execute("INSERT INTO trade VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                               (trade_id, goods_id, category_name + "｜" + goods_name, description, use_way, card_id,
                                card_content, user_id, username, now_time, 'unpaid', '', 'epay'))
                conn.commit()
                conn.close()
...
def submit_trade_usdt(update, context):
    print("选择了USDT提交订单")
    query = update.callback_query
    query.answer()
    user = update.callback_query.message.chat
    ...
    try:
        ...
        if trade_list is None:
            ...
            now_time = int(time.time())
            trade_data = make_data_dict(price, name, trade_id)
            pay_data = epusdt_submit(trade_data)

            if pay_data['status_code'] == 200:
                payment_url = pay_data['data']['payment_url']
                print('API请求成功，已成功返回支付链接')
                ...
                cursor.execute("INSERT INTO trade VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                               (trade_id, goods_id, category_name + "｜" + goods_name, description, use_way, card_id,
                                card_content, user_id, username, now_time, 'unpaid', pay_data['data']['trade_id'],
                                'epusdt'))
                conn.commit()
                conn.close()
                keyboard = [[InlineKeyboardButton("点击跳转支付", url=payment_url)]]
                reply_markup = InlineKeyboardMarkup(keyboard)
                query.edit_message_text(
                    '请在{}s内支付完成，超时支付会导致发货失败！\n'
                    '[点击这里]({})跳转支付，或者点击下方跳转按钮'.format(PAY_TIMEOUT, payment_url),
                    parse_mode='Markdown',
                    reply_markup=reply_markup
                )
            return ConversationHandler.END
    ...
    while True:
        ...
        for i in unpaid_list:
            ...
            if sub_time >= PAY_TIMEOUT:
                ...
                try:
                    if pay_channel == 'epay':
                        rst = epay.check_status(trade_id)
                    elif pay_channel == 'epusdt':
                        rst = epusdt.check_status(out_trade_no)
                    # 如果没支付成功，继续判断易支付的
                    if rst != '支付成功':
                        rst = epay.check_status(trade_id)
                    if rst == '支付成功':
